...
function readLangJson(lang){
  var lang_text = fs.readFileSync(path.join(appRoot.path, 'public', 'lang', lang+'.json'), function (err, data) {
    if (err) throw err;
...
router.get('/', function(req, res, next) {
  ...
  // }
  res.render('index.pug', {lang: lang_json});
});

router.get('/about', function(req, res, next) {
  var lang_json = JSON.parse(readLangJson(req.cookies.lang));
  res.render('about.pug', {lang: lang_json});
});
